...
const MyAIReports = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'generated':
        return <Badge variant="outline" className="gap-1"><Clock className="h-3 w-3" />Pending Intern Review</Badge>;
      case 'intern_verified':
        return <Badge variant="secondary" className="gap-1"><AlertCircle className="h-3 w-3" />Verified by Intern</Badge>;
      case 'doctor_verified':
        return <Badge variant="default" className="gap-1"><CheckCircle className="h-3 w-3" />Doctor Approved</Badge>;
      case 'completed':
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            No AI reports yet. Use the AI Health Assistant to generate your first report.
          </div>
        ) : (
              ...
              <div key={report.id} className="border rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div className="space-y-1 flex-1">
                    <p className="font-medium">AI Analysis from {new Date(report.created_at).toLocaleDateString()}</p>
